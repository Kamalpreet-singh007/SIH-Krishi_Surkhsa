...
const from = location.state?.from?.pathname || "/location-permission";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      await signup(name, email, password); // signup via AuthContext
      history.replace(from); // redirect to intended page (dashboard or previous)
    } catch (err) {
      setError("Failed to create an account");
    } finally {
      setIsLoading(false);
          ...
          <CardBody className="p-6 space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <Input
                ...
                value={name}
                onValueChange={setName}
                startContent={<Icon icon="lucide:user" className="text-default-400 w-4 h-4" />}
                isRequired
              />
              ...
              <div className="text-center text-sm">
                <span className="text-default-500">Already have an account? </span>
                <Link as={RouterLink} to={{ pathname: "/login", state: { from } }}>
                  Login
                </Link>
              </div>
            </form>
